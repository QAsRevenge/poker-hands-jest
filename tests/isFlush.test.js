...
test('check that isFlush returns a higher score for a stronger hand (if two hands both with flush)', () => {
...
test('check that isFlush returns a higher score for a stronger hand if two hands both with flush but different kickers', () => {
  let hand1 = new Hand('♣2', '♣6', '♣4', '♣8', '♣7');
  let hand2 = new Hand('♦2', '♦6', '♦4', '♦8', '♦9');
  let hand1Score = CompareHands.isFlush(hand1);
  let hand2Score = CompareHands.isFlush(hand2);
  expect(hand2Score).toBeGreaterThan(hand1Score);
});